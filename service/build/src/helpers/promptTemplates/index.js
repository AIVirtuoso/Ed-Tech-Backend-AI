...
Don't say things about 'provided context' either. If you don't have enough context to answer a question, helpfully suggest thay they rephrase the question as sometimes a differently-phrased question can help you find the answer.
  ...
  `;
export const homeworkHelpPrompt = (topic) => `You're an incredibly intelligent, incredibly empathetic ${topic} teacher. Your approach to teaching is to take the student through a step-by-step process, explaining the concepts to them, asking them questions to see if they understand, and when they don't, you correct them. You lead the conversation, ask pop quiz questions, make illustrations and draw parallels and consistently ask questions to gauge their understanding.
...
export const generateDocumentKeywordsPrompt = (note) => `Condense the supplied note into an array of keywords. The keywords are the most crucial words or phrases that you think is worth discussing with me, and capture the essence of the note. Each keyword should be no longer than five words.

  The shape of the array should be: ["keyword1", "keyword2", "etc"]
  
  Here is the note: ${JSON.stringify(note, null, 2)}`;
export const mnemonicPrompt = (query) => `You are a mnemonic generator. When you receive input, you try to understand the context, paying attention to the first letters in the input, and then you will come up with a catchy, memorable one-liner that helps with memorizing the input. You will then explain how the one-liner works, and the cognitive shortcuts it helps the user internalize. Your input is: ${query}. Return only A JSON response in this format: 
        ...
        `;
export const generalFlashcardPrompt = (count, difficulty, subject, topic, blacklistedQuestions) => {
    const promptForMoreQuestions = blacklistedQuestions && blacklistedQuestions.length > 0
...
export const flashCardsFromNotesPrompt = (note, count, blacklistedQuestions) => {
    const promptForMoreQuestions = blacklistedQuestions && blacklistedQuestions.length > 0
...
export const flashCardsFromDocsPrompt = (docs, count, blacklistedQuestions) => {
    const promptForMoreQuestions = blacklistedQuestions && blacklistedQuestions.length > 0
        ? `Avoid creating flashcards from these existing questions: [${blacklistedQuestions.join(', ')}].`
        : '';
...
const generateOptionsStructure = (type) => {
    if (type === 'mixed') {
...
export const generalQuizPrompt = (type = 'mixed', count, level, subject, topic) => {
