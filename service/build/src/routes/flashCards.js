...
import { generalFlashcardPrompt, flashCardsFromNotesPrompt, flashCardsFromDocsPrompt } from '../helpers/promptTemplates';
...
flashCards.post('/students/:studentId', validate(Schema.queryEmbeddingsSchema), async (req, res, next) => {
    try {
        ...
        const flashCardPrompt = generalFlashcardPrompt(count, difficulty, subject, topic, existingQuestions);
...
flashCards.post('/generate-from-plain-notes', validate(Schema.generateFromNotesSchema), async (req, res, next) => {
    try {
        ...
        if (!hasContent) {
            res.status(400).json({
                message: 'Cannot create questions for this note because the content is empty or null.'
            });
            return;
        }
        const noteData = extractTextFromJson(note.note);
        const flashCardsFromNotes = flashCardsFromNotesPrompt(noteData, count, existingQuestions);
        let topK = 50;
        const model = new OpenAI({
            temperature: 0,
            openAIApiKey: openAIconfig.apikey,
...
flashCards.post('/generate-from-notes', validate(Schema.generateFromDocsSchema), async (req, res, next) => {
    try {
        ...
        const flashCardsFromNotes = flashCardsFromDocsPrompt(JSON.stringify(docs), count, existingQuestions);
        const generateCards = async () => {
            try {
                const response = await model.call(flashCardsFromNotes);
                res.json(JSON.parse(response));
