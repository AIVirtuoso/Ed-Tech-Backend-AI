...
import { retrieveDocument, updateDocument, deleteSummary } from '../../db/models/document';
import fs from 'fs';
import paginatedFind from '../helpers/pagination';
import { summarizeNotePrompt } from '../helpers/promptTemplates/';
import { chatHasTitle, deleteConversation, getChatConversationId, storeChatTitle } from '../../db/models/conversation';
import { fetchAllStudentChats, fetchSpecificStudentChat } from '../../db/models/conversationLog';
...
const { ingest, summary, blockNotes, chatHistory, patchSummary, editHistoryTitle } = schema;
const openAIconfig = config.get('openai');
const createDocumentAndReturnFilePath = async (documentURL, documentId) => {
...
notes.get('/', async (req, res, next) => {
    try {
        ...
        if (!studentId)
            throw new Error('No studentId present in request!');
...
notes.get('/chat/history', validate(chatHistory), async (req, res, next) => {
    try {
        ...
        const chatHistory = await paginatedFind(chats, {
            studentId,
            conversationId
        }, { limit: 50 });
        const mappedChatHistory = chatHistory
            .map((history) => history.log)
            .reverse();
...
notes.get('/conversations', async (req, res, next) => {
    try {
        const { studentId, documentId } = req.query;
        const reference = documentId
            ? USER_REFERENCE.DOCUMENT
...
notes.get('/conversations/:conversationId', async (req, res, next) => {
    try {
        ...
        if (!hasTitle) {
            ...
            const mappedChatHistory = chatHistory
                .map((history) => history.log)
                .reverse();
            mappedChatHistory.forEach((message) => {
...
notes.delete('/conversations/:conversationId', async (req, res, next) => {
    try {
        const { conversationId } = req.params;
...
notes.post('/conversations/:conversationId/update', validate(editHistoryTitle), async (req, res, next) => {
    try {
        const { conversationId } = req.params;
...
notes.post('/summary/generate', validate(summary), async (req, res, next) => {
    try {
        ...
        if (!document)
            throw new Error(`No document with id ${documentId} for student with id ${studentId}`);
...
notes.get('/conversations/:conversationId/description', async (req, res, next) => {
    try {
        const { conversationId } = req.params;
        const pastMessages = [];
        let chatHistory = await fetchSpecificStudentChat(conversationId);
        ...
        const description = await generateConversationDescription(chatInfo.join('-'), memory);
        res.send({
            data: description,
            pastMessages
        });
...
notes.get('/summary', validate(chatHistory), async (req, res, next) => {
    try {
        const { studentId, documentId } = req.query;
        const referenceId = studentId;
        // @ts-ignore
        const document = await retrieveDocument({ referenceId, documentId });
        if (!document)
            throw new Error(`Cannot retrieve summary for document as it doesn't exist for this student!`);
...
notes.delete('/summary', validate(chatHistory), async (req, res, next) => {
    try {
        const { studentId, documentId } = req.query;
        const referenceId = studentId;
        // @ts-ignore
...
notes.patch('/summary', validate(patchSummary), async (req, res, next) => {
    try {
        const { studentId, documentId, summary } = req.body;
        if (!summary)
            throw new Error('No summary supplied!');
        const referenceId = studentId;
        // @ts-ignore
...
notes.post('/create', validate(blockNotes), async (req, res, next) => {
    try {
        const { document, studentId, documentId, title } = req.body;
        const documentData = req.body;
        const wrapText = document.map((document) => {
            return {
...
notes.post('/ingest', validate(ingest), async (req, res, next) => {
    try {
        ...
        const { embeddingAI: embedding, pineconeIndex } = res.app.locals;
        const filePath = await createDocumentAndReturnFilePath(documentURL, studentId);
        const pdfdata = fs.readFileSync(filePath);
        const text = await pdf(pdfdata).then((data) => data.text);
...
notes.delete('/clear', async (req, res, next) => {
    try {
        ...
        const { studentId } = req.query;
        if (!studentId)
            throw new Error('sudentId required.');
        const conversationId = await getChatConversationId({
            reference: USER_REFERENCE.STUDENT,
            // @ts-ignore
            referenceId: studentId
