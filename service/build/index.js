...
import { getChatConversationId, createNewConversation, chatHasTitle, storeChatTitle } from './db/models/conversation';
...
const socketAiModel = (socket, event, model) => {
...
docChatNamespace.on('connection', async (socket) => {
    ...
    const docChatChain = (event, topK) => {
        const model = socketAiModel(socket, event);
        const llm = new ChatOpenAI({
            openAIApiKey: apikey
        });
        const chain = ConversationalRetrievalQAChain.fromLLM(model, vectorStore.asRetriever(topK), {
    ...
    const pastMessages = chats.map((chat) => {
        if (chat.log.role === 'assistant')
            return new AIChatMessage(chat.log.content);
        if (chat.log.role === 'user')
            return new HumanChatMessage(chat.log.content);
    });
    ...
    Your answer:`;
        const callChain = async () => await chain
            .call({ question })
            .then(async (response) => {
            ...
            socket.emit(`${event} start`, 'I ran into some trouble coming up with an answer. Can you ask me the question again?');
        });
    ...
    socket.on('generate summary', async () => {
        ...
        const chain = RetrievalQAChain.fromLLM(model, vectorStore.asRetriever(TOP_K));
        try {
...
homeworkHelpNamespace.on('connection', async (socket) => {
    const { studentId, topic, subject, level, conversationId: convoId } = socket.handshake.auth;
    const event = 'chat response';
  ...
  Tutor:`;
    ...
    console.log(chats);
    const lastTenChats = chats.map((chat) => chat.log).reverse();
    const pastMessages = [];
    lastTenChats.forEach((message) => {
    ...
    socket.on('chat message', async (message) => {
        ...
        if ((!isFirstConvo && message !== CONVERSATION_STARTER_TEXT) ||
