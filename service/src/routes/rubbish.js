...
const aiPrompt = (message) => {
  return ` - Chat History: ${JSON.stringify(
    chatHistory
  )} - Document Context: The pdf document
...
async function getAIResponse(studentId, documentId, message) {
  ...
  const content =
    'module.exports = ' + JSON.stringify(convoHistory, null, 2) + ';';
...
return `
  this is a stringified JSON of history of the chat so far: ${JSON.stringify(
    chatHistory
  )}
