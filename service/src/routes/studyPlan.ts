...
eventEmitter.on(
  ...
  async ({
    ...
    syllabusData: any;
    job: any;
    // job: database.ThenableReference
  }) => {
    // const job = db.ref('syllabus-process-job').push();
    // const jobId = job.key;
    ...
    try {
      if (syllabusUrl) {
        ...
        await job.update({
          studyPlan: formattedStudyPlan.studyPlan,
          status: 'success'
        });
      }
      if (syllabusData) {
      ...
      job.set({ status: 'failed', error: error });
    }
  }
);
...
studyPlanRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const job = db.ref('syllabus-process-job').push();
      const jobId = job.key;
      job.set({ ...req.body, status: 'pending' });
      eventEmitter.emit('generateStudyPlan', {
        syllabusUrl,
        syllabusData,
        job
      });
